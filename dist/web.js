...
function getApp(loomerApp) {
    if (!loomerApp)
        throw new Error("LoomerApp not initialized");
    return loomerApp;
}
export function createApp(config, loomerApp, sseOptions) {
    const app = express();
    ...
    const activeSseIntervals = new Set();
    app.get("/api/log/:name/stream", validateNameParam, (req, res) => {
        ...
        const interval = setInterval(() => {
            try {
                ...
                const agent = getApp(loomerApp).status().find((a) => a.name === name);
    ...
    app.get("/api/diff/:name", validateNameParam, (req, res, next) => {
        try {
            const name = req.params.name;
            const mode = req.query.mode || "stat";
    ...
    app.post("/api/done/:name", validateNameParam, (req, res, next) => {
        try {
            getApp(loomerApp).done(req.params.name);
            res.json({ ok: true });
        }
        catch (err) {
            next(err);
    ...
    app.post("/api/kill/:name", validateNameParam, (req, res, next) => {
        try {
            ...
            getApp(loomerApp).kill(req.params.name, clean);
            res.json({ ok: true });
        }
        catch (err) {
            next(err);
    ...
    app.post("/api/retry/:name", validateNameParam, (req, res, next) => {
        try {
            getApp(loomerApp).retry(req.params.name);
            res.json({ ok: true });
        }
        catch (err) {
            next(err);
    ...
    app.post("/api/accept/:name", validateNameParam, (req, res, next) => {
        try {
            getApp(loomerApp).accept(req.params.name);
            res.json({ ok: true });
        }
        catch (err) {
            next(err);
    ...
    app.post("/api/reject/:name", validateNameParam, (req, res, next) => {
        try {
            getApp(loomerApp).reject(req.params.name);
            res.json({ ok: true });
        }
        catch (err) {
            next(err);
