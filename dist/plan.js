...
function fromData(data) {
    ...
    for (const t of rawTasks) {
        ...
        if (!t.prompt || typeof t.prompt !== "string") {
            throw new PlanFormatError(`Task "${t.id}" missing required 'prompt' field`);
        }
    }
    return {
        name: data.name,
...
export function parsePlan(path) {
    try {
        ...
        if (!match)
            throw new PlanFormatError("No YAML frontmatter found");
...
export function fromPrdJson(prdPath, maxConcurrent) {
    try {
        ...
        const tasks = taskSplit.map((ts) => {
            const storyId = ts.userStory;
            const story = storyMap.get(storyId);
            const basePrompt = story?.description || story?.title || `Task ${ts.id}`;
            ...
            return {
                ...
                dependsOn: ts.depends ?? [],
            };
        });
        return {
            name: prd.project ?? "unnamed",
...
export function validateDag(spec) {
    ...
    for (const task of spec.tasks) {
        if (!VALID_ID.test(task.id) || task.id.length > 64) {
            throw new PlanFormatError(`Invalid task ID: "${task.id}" (must match /^[a-zA-Z0-9_-]+$/, max 64 chars)`);
        }
    }
    ...
    for (const task of spec.tasks) {
        for (const dep of task.dependsOn) {
            if (!ids.has(dep)) {
                throw new PlanFormatError(`Missing dependency: "${dep}" referenced by "${task.id}"`);
            }
        }
    }
    // 规则 4: 无环（Kahn 算法拓扑排序）
...
export class PlanExecutor {
    ...
    launchReady() {
        ...
        for (const task of this.spec.tasks) {
            if (this.taskStatus.get(task.id) !== "PENDING")
                continue;
            if (runningCount + started.length >= this.spec.maxConcurrent)
                break;
