...
export class ProcessManager {
    ...
    // 信号处理器调用：杀所有被跟踪的进程组
    killAllProcessGroups(sig) {
        for (const [, tracked] of this.processes) {
            if (!tracked.exited && tracked.child.pid) {
                try {
                    process.kill(-tracked.child.pid, sig);
                }
                catch (err) {
                    const code = err.code;
                    if (code !== "ESRCH") {
                        console.warn(`[ProcessManager] signal handler kill(-${tracked.child.pid}) failed: ${code ?? err}`);
                    }
                }
            }
        }
    }
    start(name, worktreePath, prompt) {
        ...
        child.stderr.on("data", (chunk) => {
            ...
            console.warn(`[ProcessManager:${name}] stderr: ${chunk.toString().trim()}`);
        });
        ...
        if (tracked && !tracked.exited) {
            ...
            try {
                ...
                if (code === "ESRCH") {
                    ...
                    console.warn(`[ProcessManager] process.kill(-${pid}) failed: ${code ?? err}`);
                    tracked.child.kill("SIGTERM");
                }
            }
        }
